...
This is a personal website. It has a blog and an about page — the bar was on the floor. It also has a hand-traced, BPM-synced canvas animation, a named star that colors itself with the real weather everywhere on the site except the about page, and only dances alongside the blog's posts when there's actually music playing, and a GitHub Action that checks the weather in Boston every morning so a hidden feature can pick the right color. None of this was required. All of it shipped anyway.
